...
int OpenComport(int comport_number, int baudrate)
{
  if((comport_number>16)||(comport_number<1))
  {
    printf("illegal comport number\n");
    return(-1);
  }
  ...
  Cport[comport_number] = CreateFile(comports[comport_number-1],
  ...
  Cptimeouts.ReadTotalTimeoutConstant    = 2500;
  ...
  return(_open_osfhandle((long)Cport[comport_number], O_RDWR | O_BINARY));
}
